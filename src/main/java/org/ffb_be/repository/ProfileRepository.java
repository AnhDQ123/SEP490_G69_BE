...
public interface ProfileRepository extends JpaRepository<Profile, Long> {
    @Query("SELECT new org.ffb_be.dto.auth.ProfileDto.ProfileDTO(p.id, p.name,p.user.role.name, p.address, p.avatar,p.createdAt,p.dob,p.user.phone,p.tax_code,p.citizenIDNumber,p.citizenIDCardFront,p.citizenIDCardBack,p.citizenIDExpiredDate) " +
