...
// ✅ Move dòng này xuống cuối
    ...
    public String syncAllProducts() {
        ...
        long totalProductsSynced = 0;

        // Tiến hành đồng bộ từng trang
        do {
            dbProducts = productRepository.findAll(PageRequest.of(page, pageSize)).getContent();
            ...
            // Lưu vào Elasticsearch (không cần xóa toàn bộ index)
            // Lưu hoặc cập nhật từng sản phẩm
            productESRepository.saveAll(esProducts);

            totalProductsSynced += esProducts.size();
            page++;
        } while (dbProducts.size() == pageSize);

        return "Đã đồng bộ " + totalProductsSynced + " sản phẩm";
    }
